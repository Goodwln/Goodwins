...
float ASpiderPawn::GetIKOffsetForASocket(const FName SocketName)
{
	...
	else if (UKismetSystemLibrary::LineTraceSingle(GetWorld(), TraceStart, TraceEnd - IKTraceExtendDistance * FVector::UpVector, TraceType, true, TArray<AActor*>(), EDrawDebugTrace::ForOneFrame, HitResult, true))
	{
		Result = (TraceEnd.X - HitResult.Location.X) / IKScale;
	}
